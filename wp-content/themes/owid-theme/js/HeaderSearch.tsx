...
import { renderChartsPage } from '../src/renderPage';
...
class SearchResults extends React.Component<{ results: Results }> {
    ...
    componentDidMount() {
        autorun(() => {
            this.bestChartSlug && this.renderChart()
            this.bestChartSlug ? this.newChart = true : undefined
        })
    }

    @action.bound renderChart() {
        document.querySelectorAll("figure")!.forEach(e => e.remove());
